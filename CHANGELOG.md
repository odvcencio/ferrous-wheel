...
- `run`, `build`, and `package` now accept a directory of `.fw` files as one
  package. A `file.fw` input keeps its single-file behavior.
- Directory builds follow local imports in the nearest Go module. Go sees
  generated files at their source paths, so `go:embed` uses adjacent assets.
- Directory package manifests use schema 2 and list the source files and
